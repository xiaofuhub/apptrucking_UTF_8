...
public function integer wf_changeshipreftexts (long al_eqid, string as_oldeqref, string as_neweqref)
...
IF ll_Shipment > 0 THEN
	...
	IF lnv_Disp.of_Retrieveshipment( ll_Shipment ) = 1 THEN
		...
		END IF
		/////////////////////
		IF gnv_app.of_Getprivsmanager( ).of_GetUserpermissionfromfn( ls_privFunction , lnv_Shipment ) <> n_cst_privsmanager.ci_True THEN
			lb_Return = FALSE
		END IF		
...
public function integer wf_changeshipreftexts (long al_eqid, string as_oldeqref, string as_neweqref);//Created By dan 2-1-07
/*
	Returns 1 or -1 if something went wrong
*/


Long	ll_eqId 
Long	ll_shipId
Long	ll_shipWindowHandle
...
IF isValid(lnv_shipment) THEN
	IF lnv_shipment.of_hasSource( ) THEN
		ls_EquipmentType = dw_basics.object.eq_type [ 1 ]
		ls_EquipmentRef = as_oldEqRef
		...
		IF IsValid ( lnv_Shipment ) AND Len ( as_neweqref ) > 0 THEN
			...
			lnv_shipment.of_GetLinkedequipment( lnva_equipment )
			
			li_max = upperBound( lnva_equipment )
			IF li_max > 0 THEN	
				li_Ref1Type = lnv_Shipment.of_getRef1Type ( ) 
				ls_ref1Text = lnv_Shipment.of_GetRef1text ( ) 
				lb_doRef1 = (li_Ref1type = 0 OR isNull ( li_Ref1Type ) ) OR (  li_SetType = li_Ref1Type AND ( isNull ( ls_Ref1Text ) OR ls_Ref1Text = as_oldeqref ) )
			END IF
			
			IF li_max > 1 THEN
				li_Ref2Type = lnv_Shipment.of_getRef2Type ( ) 
				ls_ref2Text = lnv_Shipment.of_GetRef2text ( ) 
				lb_doRef2 = (li_Ref2type = 0 OR isNull ( li_Ref2Type ) ) OR (  li_SetType = li_Ref2Type AND ( isNull ( ls_Ref2Text ) OR ls_Ref2Text = as_oldeqref ) )
			END IF
			
			IF li_max > 2 THEN
				li_Ref3Type = lnv_Shipment.of_getRef3Type ( ) 
				ls_ref3Text = lnv_Shipment.of_GetRef3text ( ) 
				lb_doRef3 = (li_Ref3type = 0 OR isNull ( li_Ref3Type ) ) OR (  li_SetType = li_Ref3Type AND ( isNull ( ls_Ref3Text ) OR ls_Ref3Text = as_oldeqref ) )
			END IF
	
			IF li_Return = 1 THEN
				
				IF li_SetType > 0 THEN
					//li_res = 1  DEK THIS NEEDS TO BE REVISITED 3-28-07, for now this code will
					//never be executed, but it probably has to be redone anyways.
					IF li_res = 1  THEN
						IF lb_doRef1 THEN
							lnv_Shipment.of_SetRef1Type( li_SetType )
							lnv_Shipment.of_SetRef1Text ( as_neweqref )
						END IF
						...
						IF lb_doRef3 THEN
							lnv_Shipment.of_SetRef3Type( li_SetType )
							lnv_Shipment.of_SetRef3Text ( as_neweqref )
						END IF
						
						//when we create a beo out of thin air, its because this window wasn't opened from
...
FOR li_index = 1 TO li_max
	DESTROY lnva_equipment[li_index]
NEXT
